    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#five_star_progress").css(
          "width",
          (parsedData.totalRatings5 / parsedData.totalReviews) * 100
        );
        $("#four_star_progress").css(
          "width",
          (parsedData.totalRatings4 / parsedData.totalReviews) * 100
        );
        $("#three_star_progress").css(
          "width",
          (parsedData.totalRatings3 / parsedData.totalReviews) * 100
        );
        $("#two_star_progress").css(
          "width",
          (parsedData.totalRatings2 / parsedData.totalReviews) * 100
        );
        $("#one_star_progress").css(
          "width",
          (parsedData.totalRatings1 / parsedData.totalReviews) * 100
        );
        ...
        if (parsedData.ratingsList.length > 0) {
          ...
          for (var count = 0; count < parsedData.ratingsList.length; count++) {
            ...
            html += `<h1><div class='bg-danger rounded-circle text-center text-white text-uppercase pt-2 pb-2'>${parsedData.ratingsList[
              count
            ].name.charAt(0)}</div></h1>`;
            html += `</div>`;
